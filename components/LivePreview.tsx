...
const LivePreview: React.FC<LivePreviewProps> = ({ code, projectName }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50 dark:bg-[#0b0e14] rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-white/10 relative">
      ...
      <div className="h-14 border-b border-slate-200 dark:border-white/5 flex items-center justify-between px-6 bg-white dark:bg-[#080a0f] z-20 rounded-t-[inherit]">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-100 dark:bg-[#151b28] border border-slate-200 dark:border-white/5 px-3 py-1.5 rounded-xl gap-2 min-w-[120px] md:min-w-[300px]">
            <Lock size={10} className="text-slate-400 dark:text-slate-500" />
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono tracking-tight truncate max-w-[100px] md:max-w-none font-bold">{slugifiedName}.ai-dev-team.app</span>
          </div>
      ...
      {/* Viewport Container */}
      <div className="flex-1 overflow-auto bg-slate-100 dark:bg-[#0b0e14] flex justify-center p-4 md:p-10 custom-scrollbar relative">
        ...
        <div className={`transition-all duration-500 ease-in-out shadow-2xl relative bg-white dark:bg-[#0b0e14] border border-slate-200 dark:border-white/5 overflow-hidden ${
          isMobile ? 'w-[375px] h-[812px] rounded-[3.5rem] border-[12px] border-white dark:border-[#151b28]' : 'w-full max-w-5xl rounded-3xl'
        }`}>
          ...
          <div className={`h-full flex flex-col text-slate-900 dark:text-slate-200 font-sans overflow-y-auto custom-scrollbar bg-white dark:bg-[#0b0e14] ${
            isMobile ? 'rounded-[2.8rem]' : 'rounded-3xl'
          }`}>
            
            {/* Nav Header */}
            <nav className={`flex items-center justify-between sticky top-0 bg-white/95 dark:bg-[#0b0e14]/95 backdrop-blur-md z-30 ${isMobile ? 'px-8 py-5 rounded-t-[2.8rem]' : 'px-10 py-8 rounded-t-3xl'}`}>
              ...
              <div className="flex items-center gap-4">
                {isMobile ? (
                  ...
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center border-2 border-white dark:border-[#0b0e14] shadow-lg">
            ...
            {/* Hero Section */}
            <section className={`text-center mx-auto space-y-8 flex flex-col items-center ${isMobile ? 'px-8 pt-16 pb-10' : 'px-10 pt-24 pb-20 max-w-2xl'}`}>
              <h2 className={`font-black text-slate-900 dark:text-white tracking-tighter leading-tight ${isMobile ? 'text-4xl' : 'text-5xl md:text-7xl'}`}>
              ...
              <p className={`text-slate-500 dark:text-slate-500 font-bold leading-relaxed ${isMobile ? 'text-xs px-4' : 'text-sm md:text-lg opacity-80'}`}>
                AI-generated full-stack application with integrated modules, built by the agent swarm pipeline.
              </p>
              <div className="pt-6 w-full">
                <button className={`bg-gradient-to-br from-indigo-600 to-purple-600 hover:brightness-110 text-white rounded-[1.25rem] font-black flex items-center justify-center gap-3 mx-auto shadow-2xl shadow-indigo-500/30 transition-all active:scale-95 group ${isMobile ? 'w-full py-5 text-sm' : 'px-10 py-4'}`}>
            ...
            {/* Stats Grid */}
            <section className={`grid gap-5 max-w-5xl mx-auto w-full ${isMobile ? 'px-8 py-8 grid-cols-2' : 'px-10 py-12 grid-cols-2 lg:grid-cols-4'}`}>
              {stats.map((stat, i) => (
                <div key={i} className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 p-6 rounded-[2rem] flex flex-col items-center justify-center gap-1.5 group hover:border-indigo-500/30 transition-all shadow-sm">
                  <div className={`font-black text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors ${isMobile ? 'text-2xl' : 'text-3xl'}`}>{stat.value}</div>
            ...
            <section className={`max-w-5xl mx-auto w-full space-y-8 ${isMobile ? 'px-8 py-10' : 'px-10 py-20'}`}>
            ...
            {/* Footer */}
            <footer className={`mt-auto border-t border-slate-100 dark:border-white/5 ${isMobile ? 'px-8 py-12' : 'px-10 py-16'}`}>
